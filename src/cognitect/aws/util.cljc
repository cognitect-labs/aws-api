...
(ns ^:skip-wiki cognitect.aws.util
  ...
  (:require [clojure.string :as str]
            ...
            #?(:cljs
              [goog.crypt.base64 :as base64 ;:refer [encodeString decodeString]
               ])
            #?(:cljs
              [goog.crypt]))
  #?(:clj
...
#?(:clj
   ...
   (defn format-date
     ([fmt]
      (format-date fmt (cljs-time/now)))
     ([^ThreadLocal fmt inst]
      (cljs-time.format/unparse fmt inst))))

(defn format-timestamp
  "Format a timestamp in milliseconds."
  [inst]
  (str (long (/ #?(:clj (.getTime ^Date inst)
                   :cljs (cljs-time.coerce/to-long 1000))))))
...
(defn char-array->string [ca]
  #?(:clj
     (String. ca)
     :cljs
     (clojure.string/join nil ca)))
  ...
  #?(:clj
     (cond (string? data)
           ...
           (let [digest (MessageDigest/getInstance "SHA-256")]
             ...
             (.digest digest)))
     ;; See https://stackoverflow.com/a/18848073/9800639
     :cljs
...
#?(:clj
   ...
   :cljs
   ;; Copied from https://stackoverflow.com/questions/54316829/how-to-fix-encryption-of-sha256-hmac-on-google-closure-library
   (defn hma-sha-246 [key message]
...
(defn xml-read
  ...
  [s]
  #?(:clj (xml/parse (ByteArrayInputStream. (.getBytes ^String s "UTF-8"))
                     :namespace-aware false
                     :skip-whitespace true)
     :cljs (xml/parse-str s)))
...
(defn base64-decode
  ...
  #?(:clj
     ...
     :cljs
     (js->clj (base64/decodeString s))))

(defn encode-jsonvalue [data]
  (base64-encode
   #?(:clj (.getBytes ^String (json/write-str data))
      :cljs (json/write-str data))))
